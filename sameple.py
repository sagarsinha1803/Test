...
def _build_device_context(
    ...
    if brand_model in commands_dict:
        ...
        # (If you want empty dict to behave as only-ping, change to: only_ping_check = not commands)
...
# ── Banner pattern lists (all lowercase for matching) ──────────────────────
...
async def _handle_interactive_login_banner(
    ...
    Fixes applied vs original:
      1. Lowercase buf once per iteration — case-insensitive matching.
      2. asyncio.sleep(0.5) after each keypress — let device process input.
      3. Require prompt to appear twice consecutively — avoid false-positive exits.
      4. asyncio.sleep(0.5) before loop — give device time to send initial banner.
      5. Final asyncio.sleep(0.3) settle — device fully ready before commands.
    """
    ...
    try:
        ...
        # FIX 4: give device a moment to send the initial banner
        await asyncio.sleep(0.5)

        consecutive_prompt_hits = 0  # FIX 3: require stable prompt
        ...
        while True:
            ...
            # Read a chunk (non-blocking via timeout)
            try:
            ...
            if chunk:
                ...
                # FIX 1: normalise to lowercase once for all pattern checks
                buf_lower = buf.lower()

                # ── Handle "(Press 'a' to accept)" ──────────────────────
                if any(p in buf_lower for p in _ACCEPT_A_PATTERNS):
                    ...
                    # FIX 2: wait for device to process the keypress
                    await asyncio.sleep(0.5)
                    buf = ""
                    consecutive_prompt_hits = 0
                    continue

                # ── Handle "press any key / enter" ──────────────────────
                if any(p in buf_lower for p in _PRESS_ANY_KEY_PATTERNS):
                    logger.info(
                        f"[{device_ip}] Detected 'press any key/enter' "
                        f"banner → sending ENTER"
                    )
                    proc.stdin.write("\n")
                    await proc.stdin.drain()
                    # FIX 2: wait for device to process the keypress
                ...
                # FIX 3: require prompt to appear TWICE (stable prompt check)
                if _PROMPT_RE.search(buf):
                    consecutive_prompt_hits += 1
                    if consecutive_prompt_hits >= 2:
                        logger.info(
                            f"[{device_ip}] Stable prompt detected, "
                            f"banner handling complete"
                        )
                        break
                    # wait one more read cycle to confirm stability
                ...
                # No data — small sleep to avoid tight spin
    ...
    # FIX 5: extra settle time so device is fully ready before
    # _connect_device() proceeds to "terminal length 0" and commands
    await asyncio.sleep(0.3)
...
async def _connect_device(
    ...
    # Handle interactive banners that require input (e.g. Press 'a' to accept)
    await _handle_interactive_login_banner(
        conn, ctx, timeout=min(12, max(6, connect_timeout))
    )

    # Disable paging (best-effort)
    # FIX: retry "terminal length 0" once — first attempt may still hit a
    # lingering banner prompt on some devices
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                await asyncio.sleep(1)  # wait and retry once
...
async def _run_command(
    ...
    # FIX: strip whitespace to avoid silent failures from padded commands
    cmd = cmd.strip()
    if not cmd:
        return ""
    ...
    # FIX: strip echoed command from first line (some devices echo it back,
    # which pollutes the parser input)
            ...
            try:
                if len(commands_list) == 1:
                    raw_output = await _run_command(
                        conn, commands_list[0], timeout=read_timeout
                    )
                else:
                    raw_output = await _run_multiline_commands(
                        conn, commands_list, timeout=read_timeout
                    )
